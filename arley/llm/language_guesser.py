...
class LangDetect(BaseModel):
    ...
    lang: Literal["de", "en"] = Field(
        description="either 'de' for detected language being german or 'en' for detected language being english."
    )
...
class LanguageGuesser:
    ...
    def guess_language(
        ...
        max_retries: int = 3,
    ) -> Literal["de", "en"] | tuple[Literal["de", "en"], dict, dict] | None:
    ...
    @staticmethod
    def _get_guess_language_priming_history(
        new_content: str | None = None, include_examples: bool = False
    ) -> List[Message]:
        msgs: List[Message] = []
        ...
        if include_examples:
            inputs: list[str] = [f"Hallo, wie geht's?!", f"What a nice idea!"]
            langs: list[str] = ["de", "en"]
        ...
        if new_content:
            msgs.append(Message(role="user", content=new_content))
