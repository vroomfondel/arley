...
readonly POSTGRES_VERSION=18
pgvector_version=$(curl -s "https://api.github.com/repos/pgvector/pgvector/tags" | jq -r '.[0].name')
readonly PGVECTOR_VERSION=${pgvector_version}
readonly DOCKER_IMAGE="docker.io/xomoxcc/postgreslocaled:${POSTGRES_VERSION}-${DEBIAN_VERSION}-pgvector-${PGVECTOR_VERSION#v}"
readonly DOCKER_IMAGE_LATEST="${DOCKER_IMAGE%:*}:latest"
readonly PLATFORMS=("linux/amd64" "linux/arm64")
readonly DOCKERFILE=Dockerfile_postgres
